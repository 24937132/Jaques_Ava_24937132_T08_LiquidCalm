...
# ---------- Setup ----------
 ...
 #Set canvas size (width, height) and initialise all elements
# Sets the target frame rate (how many times draw() runs per second).
# Creates the initial set of bubbles.
...
# ---------- Sand ----------
#Draws a horizontal rectangle of sand along the bottom.
...
def drawOneSeaweed(x, y, tall, seaweed_color):
    ...
    # sin() makes smooth back-and-forth movement, frameCount increases automatically every frame — so the angle changes slightly each time, creating motion and multiplying by 8 controls how far it sways left and right.
    
    fill(seaweed_color)   # Set the colour used to fill the seaweed leaves 
    noStroke()            # Turn off outlines around shapes so the seaweed looks smooth and natural

# Draw one seaweed plant as a stack of 5 oval (ellipses)
# Each oval is drawn a little higher than the previous one, forming a tall plant shape.
...
def drawOneCoral(x, y, size, coral_color):
    ...
    fill(coral_color)   # Set the fill colour for the coral (e.g., pink or orange)
    noStroke()          # Turn off outlines so the coral shapes blend smoothly

#Build the coral out of overlapping circles (ellipses)
# Each ellipse represents one "bulb" or branch of the coral.
# Their positions are slightly offset to create a branching effect.
# The main base circle at the bottom
# x, y → starting position (bottom of the coral)
# size * 0.8 → make it slightly smaller than the full size value
...
def init_bubbles(n=15, from_sand=True):
    global bubbles 
    bubbles = []
    for _ in range(n):     # Repeat this loop n times to create 'n' new bubbles
...
class Bubble:
    ...
    def __init__(self, x, y, r):
        ...
        self.speed = random(0.6, 1.6)  # Random upward speed, so some rise faster than others
